...
module.exports = async (client, oldMember, newMember) => {
    ...
    try {
        ...
        if (newMember.pending === false) {
            const niceone = newMember.guild.roles.cache.find(
                (role) => role.name === "Nice One"
            ).id;
            const mitglied = newMember.guild.roles.cache.find(
            ...
            if (
                await db(
                    `SELECT dcid FROM discord WHERE niceone != 1 AND dcid = ${newMember.user.id}`
                ).then((res) => res[0])
            )
                return client.channels.cache
                    .get(modlog)
                    .send(
                        "Serverregeln akzeptiert: <@" +
                            newMember.user.id +
                            ">, `" +
                            newMember.user.id +
