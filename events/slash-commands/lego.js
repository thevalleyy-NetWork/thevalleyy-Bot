...
function cooldown(interaction) {
    ...
    if (cooldownSet.has(interaction.user.id)) {
        interaction.reply( {content: '⏳ Dieser Command kann nur alle 20 Sekunden verwendet werden. ⏳', ephemeral: true} )
        return true
    }
...
module.exports = async (client, interaction) => {
    ...
    if (interaction.options._subcommand == "set") {
        ...
        fetch(`https://brickset.com/api/v3.asmx/getSets?apikey=${config.keys.brickset}&userHash=${userhash}&params={"query":"${interaction.options._hoistedOptions[0].value}"}`).then(async response =>
    ...
    if (interaction.options._subcommand == "subtheme") {
        ...
        fetch(`https://brickset.com/api/v3.asmx/getThemes?apikey=${config.keys.brickset}`).then(async response =>
            ...
            // it wasn't found
            if (foundarr.length == 0) {
                interaction.reply("Es wurde kein Theme gefunden, zu dem Unterthemen angezeigt werden können.\nMit `/lego theme` bekommst du alle Themes angezeigt.")
                return
            }
            ...
            fetch(`https://brickset.com/api/v3.asmx/getSubthemes?apikey=${config.keys.brickset}&theme=${foundarr[0].theme}`).then(async response =>
                ...
                if (json.matches == 1 && json.subthemes[0].subtheme.toLowerCase() == "{none}") {
                    interaction.reply(`Zu dem Theme \`${foundarr[0].theme}\` gibt es keine Unterthemen.\nFür mehr Informationen nutze \`/lego theme ${foundarr[0].theme}\`.`)
                    return
                }
    ...
    if (interaction.options._subcommand == "year") {
        ...
        fetch(`https://brickset.com/api/v3.asmx/getThemes?apikey=${config.keys.brickset}`).then(async response =>
            ...
            // it wasn't found
            if (foundarr.length == 0) {
                interaction.reply("Es wurde kein Theme gefunden, zu dem Infos angezeigt werden können.\nMit `" + message.content.split(" ")[0] + " Themes` bekommst du alle Themes angezeigt.")
                return
            }
    ...
    if (interaction.options._subcommand == "instructions") {
        ...
        fetch(`https://brickset.com/api/v3.asmx/getInstructions2?apikey=${config.keys.brickset}&setNumber=${interaction.options._hoistedOptions[0].value}`).then(async response =>
            response.json()).then(json => {
            if (json.status == "error") {
                //ERROR
                interaction.reply("Es gab einen Fehler beim Abrufen der Daten.")
