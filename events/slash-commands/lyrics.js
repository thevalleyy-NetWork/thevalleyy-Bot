...
module.exports = async(client, interaction) => {
    ...
    try {
        ...
        const waitEmbed = new Discord.EmbedBuilder()
            ...
            const { title, author, lyrics, thumbnail, links, error } = await fetch(`https://some-random-api.ml/lyrics?title=${interaction.options.getString("song").replaceAll('ä', 'ae').replaceAll('ü', 'ue').replaceAll('ö', 'oe').replaceAll('ß', 'ss')}`).then(response => response.json())
            ...
            const songEmbed = new Discord.EmbedBuilder()
                .setAuthor({ name: `${author} (${title})`, iconURL: interaction.user.avatarURL(), url: links.genius })
