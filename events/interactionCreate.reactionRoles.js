module.exports = async (client, interaction) => {
    ...
    const modlog = '822575095721099304'
        ...
        try {
            ...
            const botRole = interaction.message.guild.roles.cache.find(role => role.name === 'BotNews').id
            ...
            if (interaction.customId === 'REACTIONROLE_add.news') {
                ...
                client.channels.cache.get(modlog).send('<@' + interaction.user.id + '> hat nun die News Rolle bekommen. \n Seine ID: `' + interaction.user.id + '`, `' + interaction.user.createdAt.toLocaleString() + '`')
                interaction.deferUpdate()
            }

            if (interaction.customId === 'REACTIONROLE_add.botNews') {
                user.roles.add(botRole)
                client.channels.cache.get(modlog).send('<@' + interaction.user.id + '> hat nun die BotNews Rolle bekommen. \n Seine ID: `' + interaction.user.id + '`, `' + interaction.user.createdAt.toLocaleString() + '`')
                interaction.deferUpdate()
            }

            if (interaction.customId === 'REACTIONROLE_add.minecraft') {
                user.roles.add(mcRole)
                client.channels.cache.get(modlog).send('<@' + interaction.user.id + '> hat nun die Minecraft Rolle bekommen. \n Seine ID: `' + interaction.user.id + '`, `' + interaction.user.createdAt.toLocaleString() + '`')
                interaction.deferUpdate()
            }
            ...
            if (interaction.customId === 'REACTIONROLE_remove.news') {
                ...
                client.channels.cache.get(modlog).send('<@' + interaction.user.id + '> wurde nun die News Rolle entzogen. \n Seine ID: `' + interaction.user.id + '`, `' + interaction.user.createdAt.toLocaleString() + '`')
                interaction.deferUpdate()
            }

            if (interaction.customId === 'REACTIONROLE_remove.botNews') {
                user.roles.remove(botRole)
                client.channels.cache.get(modlog).send('<@' + interaction.user.id + '> wurde nun die BotNews Rolle entzogen. \n Seine ID: `' + interaction.user.id + '`, `' + interaction.user.createdAt.toLocaleString() + '`')
                interaction.deferUpdate()
            }

            if (interaction.customId === 'REACTIONROLE_remove.minecraft') {
                user.roles.remove(mcRole)
                client.channels.cache.get(modlog).send('<@' + interaction.user.id + '> wurde nun die Minecraft Rolle entzogen. \n Seine ID: `' + interaction.user.id + '`, `' + interaction.user.createdAt.toLocaleString() + '`')
                interaction.deferUpdate()
            }
