/* ******************************************** */
/* https://github.com/Gerpea/pi/blob/main/pi.js */
/* ******************************************** */
...
export default async function computePI(digits) {
    if (digits <= 0) {
        return "0";
    }
...
async function computePQT(n1, n2) {
    ...
    if (n1.plus(1).isEqualTo(n2)) {
        ...
        let res1 = await new Promise((resolve) => process.nextTick(async () => resolve(await computePQT(n1, m))));
        let res2 = await new Promise((resolve) => process.nextTick(async () => resolve(await computePQT(m, n2))));
